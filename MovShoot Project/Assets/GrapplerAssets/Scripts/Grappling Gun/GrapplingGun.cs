...
public class GrapplingGun : MonoBehaviour {
    ...
    {
        UserInputManager.Instance.Controls.Player.Attack.started += OnGrappleStart;
        UserInputManager.Instance.Controls.Player.Attack.canceled += OnGrappleEnd;
    }

    private void OnDisable()
    {
        UserInputManager.Instance.Controls.Player.Attack.started -= OnGrappleEnd;
        UserInputManager.Instance.Controls.Player.Attack.canceled -= OnGrappleEnd;
    }

    private void OnGrappleStart(InputAction.CallbackContext ctx)
    {
        Grappling = true;
    }

    private void OnGrappleEnd(InputAction.CallbackContext ctx)
    {
        Grappling = false;
    }
    ...
    private void Start () {
        _playerMovement = transform.parent.GetComponentInParent<PlayerMovement1>();
    ...
    private void Update() {
        if (Grappling && CurrentGrapplePhase == GrapplePhase.Waiting) {
